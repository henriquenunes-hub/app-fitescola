...
def avaliar_senta_alcanca_combinado(val_dta_raw, val_esq_raw, idade, genero):
    ...
    # Validação estrutural caso faltem dados num dos lados
    if classe_dta == "N/A" or classe_esq == "N/A":
        try:
            df_ref = ref_fem if str(genero).upper() in ['F', 'FEMININO', 'RAPARIGA', 'FEM'] else ref_masc
            idade_busca = max(9, min(20, int(idade)))
            linha = df_ref[df_ref['Idade'] == idade_busca].iloc[0]
            zs = linha["SA_ZS"]
            pa = linha["SA_PA"]
            ...
            zs, pa = 0, 0
        return "N/A", "Resultados inválidos ou em falta.", 0, 0, zs, pa, classe_dta, classe_esq

    # Formata a string multicélula dos resultados
    texto_resultados = f"Dir: {val_dta} cm\nEsq: {val_esq} cm"
    
    # Lógica de agrupamento de feedbacks (Não repetir se as zonas forem iguais)
    if classe_dta == classe_esq:
        texto_feedbacks = fb_dta
    else:
        texto_feedbacks = f"Perna Direita ({classe_dta}): {fb_dta}\n\nPerna Esquerda ({classe_esq}): {fb_esq}"
        
    # Verificação de assimetria clínica (Diferença estrita superior a 4 cm)
...
fig, ax = plt.subplots(figsize=(1.5, 0.4)) # Valores pequenos em polegadas
    ...
    ax.scatter([zs], [1.0], color='#2ecc71', s=50, zorder=2) 
    ax.scatter([pa], [1.0], color='#3498db', s=50, zorder=2) 
    ...
    ax.text(zs, 1.90, f'ZS:{zs}', color='#27ae60', fontsize=8, ha='center', weight='bold') 
    ax.text(pa, 1.90, f'PA:{pa}', color='#2980b9', fontsize=8, ha='center', weight='bold') 
...
def criar_pdf(aluno, resultados, data_teste, nome_professor):
    ...
    for nome_exibicao, sigla in mapa_testes.items():
        # CASO ESPECIAL: Senta e Alcança (Agrupado na mesma célula horizontal)
        if sigla == "SA":
            if "SA" in resultados:
                texto_res, fb, val_dta, val_esq, zs, pa, classe_dta, classe_esq = resultados["SA"]
                
                def obter_txt_classe(c):
                    if c == "PA": return "Perfil Atlético"
                    elif c == "ZS": return "Zona Saudável"
                    elif c == "N/A": return "N/A"
                    else: return "Abaixo Zona Saudável"
                
                txt_classe = f"Dir: {obter_txt_classe(classe_dta)}\nEsq: {obter_txt_classe(classe_esq)}"
                
                linhas_necessarias_fb = len(pdf.multi_cell(w_feedback, 4.5, fb, split_only=True))
                linhas_necessarias_cl = len(pdf.multi_cell(w_classif, 4.5, txt_classe, split_only=True))
                linhas_necessarias_res = len(pdf.multi_cell(w_resultado, 4.5, texto_res, split_only=True))
                
                linhas_necessarias = max(linhas_necessarias_fb, linhas_necessarias_cl, linhas_necessarias_res)
                altura_linha = max(28, linhas_necessarias * 4.5)  # Garante altura para os 2 gráficos
                
                x_atual = pdf.get_x()
                y_atual = pdf.get_y()
                
                # Desenhar molduras perfeitas
                pdf.rect(x_atual, y_atual, w_teste, altura_linha)
                pdf.rect(x_atual + w_teste, y_atual, w_resultado, altura_linha)
                pdf.rect(x_atual + w_teste + w_resultado, y_atual, w_classif, altura_linha)
                pdf.rect(x_atual + w_teste + w_resultado + w_classif, y_atual, w_grafico, altura_linha)
                pdf.rect(x_atual + w_teste + w_resultado + w_classif + w_grafico, y_atual, w_feedback, altura_linha)
                
                # Preencher Célula 1: Nome do Teste
                pdf.set_xy(x_atual + 1, y_atual + (altura_linha - 4.5)/2)
                pdf.cell(w_teste - 2, 4.5, nome_exibicao, 0, 0, 'L')
                
                # Preencher Célula 2: Resultados
                pdf.set_xy(x_atual + w_teste, y_atual + (altura_linha - (linhas_necessarias_res * 4.5)) / 2)
                pdf.multi_cell(w_resultado, 4.5, texto_res, 0, 'C')
                
                # Preencher Célula 3: Classificações
                pdf.set_xy(x_atual + w_teste + w_resultado, y_atual + (altura_linha - (linhas_necessarias_cl * 4.5)) / 2)
                pdf.multi_cell(w_classif, 4.5, txt_classe, 0, 'C')
                
                # Preencher Célula 4: Dois Gráficos Empilhados
                if classe_dta != "N/A" and classe_esq != "N/A":
                    img_path_dta = gerar_grafico_linha(val_dta, zs, pa, "SA_Dta", aluno['Género'])
                    img_path_esq = gerar_grafico_linha(val_esq, zs, pa, "SA_Esq", aluno['Género'])
                    arquivos_deletar.extend([img_path_dta, img_path_esq])
                    
                    h_img = (altura_linha - 5) / 2
                    pdf.image(img_path_dta, x=x_atual + w_teste + w_resultado + w_classif + 1, y=y_atual + 1, w=w_grafico - 2, h=h_img)
                    pdf.image(img_path_esq, x=x_atual + w_teste + w_resultado + w_classif + 1, y=y_atual + 1 + h_img + 2, w=w_grafico - 2, h=h_img)
                
                # Preencher Célula 5: Feedback Unificado
                pdf.set_xy(x_atual + w_teste + w_resultado + w_classif + w_grafico, y_atual + (altura_linha - (linhas_necessarias_fb * 4.5)) / 2)
                pdf.multi_cell(w_feedback, 4.5, fb, 0, 'L')
                
                pdf.set_xy(x_atual, y_atual + altura_linha)
                
        # CASO PADRÃO: Testes regulares com uma só coluna
        else:
...
if link_sheets:
    try:
        ...
        # PROCESSAMENTO E LIMPEZA DE CABEÇALHO DUPLO (Protocolo Dta/Esq)
        if len(df_alunos) > 0 and df_alunos.iloc[0].astype(str).str.contains('Dta|Esq').any():
            colunas = list(df_alunos.columns)
            if len(colunas) > 9:
                colunas[9] = "SA_Dta"
            if len(colunas) > 10:
                colunas[10] = "SA_Esq"
            df_alunos.columns = colunas
            df_alunos = df_alunos.drop(0).reset_index(drop=True)
        ...
        if col1.button("🔄 1. Analisar Dados & Gerar Relatórios"):
            ...
            with st.spinner("A modelar silhuetas e a exportar PDFs..."):
                for index, aluno in df_alunos.iterrows():
                    ...
                    # Chamada da nova rotina inteligente unificada
                    res_aluno["SA"] = avaliar_senta_alcanca_combinado(aluno.get("SA_Dta", 0), aluno.get("SA_Esq", 0), aluno['Idade'], aluno['Género'])
